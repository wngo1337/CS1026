...
def keyReader():
    fileName = input("Please enter the name of the keywords file: ")
    try:
...
def tweetReader():
    fileName2 = input("Please enter the name of the tweets file: ") # input error may occur on this line
    try:
    ...
    for tweet in tweetFile:
        ...
        if totalHappiness > 0 and latitude >= 24.660845 and latitude <= 49.189787:
            ...
            if longitude <= -67.444574 and longitude >= -87.518395:
                ...
                pass # tweet doesn't have the right longitude or has no keywords

    # calculate the happiness scores of each region
    ...
    # assigning the keys of our dictionary to lists so we can reference them later in loop
    zoneValues["pacific"] = []
    zoneValues["mountain"] = []
    zoneValues["central"] = []
    zoneValues["eastern"] = []
...
def main():
    ...
    for key in zoneValues:
        print("The happiness score for the {} timezone is {}, and we found {} tweets in its range".format\
              (key, zoneValues[key][0], zoneValues[key][1]))

    # calling the drawSimpleHistogram function from happy_histogram
    happy_histogram.drawSimpleHistogram\
        (zoneValues["pacific"][0], zoneValues["mountain"][0], zoneValues["central"][0], zoneValues["eastern"][0])
